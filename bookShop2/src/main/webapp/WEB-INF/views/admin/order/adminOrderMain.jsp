				...
				<tr>
				  <td>
				    <select name="s_search_type" disabled >
						...
						<option value="orderer_name">예약자이름</option>
						<option value="orderer_id">예약자아이디</option>
						<option value="orderer_hp">예약자휴대폰번호</option>
						<option value="orderer_goods">가게명</option>
					</select>
            ...
            <c:choose>
              ...
              <c:otherwise>
				...
				<td width=50% align=left >
				    <strong>예약자:${item.orderer_name}</strong><br>
				  <strong>예약 전화번호:${item.orderer_hp}</strong><br>				 
				  <strong>예약 휴대 전화번호:${item.receiver_hp1}-${item.receiver_hp2}-${item.receiver_hp3}</strong><br>
				  <strong>업체명:${item.goods_title}(${item.order_goods_qty})</strong><br>
				     <c:forEach var="item2" items="${newOrderList}" varStatus="j">
				       <c:if test="${j.index > i.index }" >
				          <c:if  test="${item.order_id ==item2.order_id}" >
				            <strong>업체명:${item2.goods_title}(${item2.order_goods_qty})</strong><br>
				      </c:if>   
				       </c:if>
				    </c:forEach> 
				</td>
				<td width=10%>
				 ...
				 <c:choose>
				   <c:when test="${item.delivery_state=='delivery_prepared' }">				     			   
				     <option  value="delivering" selected >예약중</option>
				     <option  value="finished_delivering">사용완료</option>
				     <option  value="cancel_order">예약취소</option>
				     
				   </c:when>
				   <c:when test="${item.delivery_state=='finished_delivering' }">
				   
				     <option  value="delivering"  >예약중</option>
				     <option  value="finished_delivering" selected>사용완료</option>
				     <option  value="cancel_order">약취소</option>
				   ...
				   <c:when test="${item.delivery_state=='cancel_order' }">
				     ...
				     <option  value="delivering"  >예약중</option>
				     <option  value="finished_delivering" >사용완료</option>
				     <option  value="cancel_order" selected>예약취소</option>				    
				   </c:when>
				...
				<td width=10%>
			     <input  type="button" value="정보수정"  onClick="fn_modify_order_state('${item.order_id}','s_delivery_state${i.index}')"/>
			    </td>
			</tr>
		</c:when>
		</c:choose>	
