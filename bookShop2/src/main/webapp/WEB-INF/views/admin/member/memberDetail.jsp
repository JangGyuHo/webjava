				...
				<tr class="dot_line">
					...
					<c:choose> 
					   <c:when test="${member_info.emailsts_yn=='true' }">
					     <input type="checkbox" name="emailsts_yn"  value="Y" checked /> 영화예매사이트에서 발송하는 e-mail을 수신합니다.
						</c:when>
						<c:otherwise>
						  <input type="checkbox" name="emailsts_yn"  value="N"  /> 영화예매사이트에서 발송하는 e-mail을 수신합니다.
						</c:otherwise>
					 </c:choose>
					</td>
					<td>
