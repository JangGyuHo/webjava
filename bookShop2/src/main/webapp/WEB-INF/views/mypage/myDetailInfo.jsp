...
function fn_delete_member(member_id ,del_yn){
    ...
    frm_mod_member.action="/bookshop01/mypage/deleteMember.do";
...
#detail_table table tr.dot_line {
	border-bottom-color: rgb(102, 102, 102); border-bottom-width: 1px; border-bottom-style: none;
}
input {
margin-right:5px;
}
	...
	<div id="detail_table">
					...
					<td>
					 ...
					 <c:choose> 
					   <c:when test="${memberInfo.smssts_yn=='true' }">
					     <input type="checkbox"  name="smssts_yn" value="Y" checked /> 본 사이트에서 발송하는 SMS 소식을 수신합니다.
						</c:when>
						<c:otherwise>
						  <input type="checkbox"  name="smssts_yn" value="N"  /> 본 사이트에서 발송하는 SMS 소식을 수신합니다.
						</c:otherwise>
					 </c:choose>	
				    </td>
					<td>
				...
				<tr class="dot_line">
					...
					<c:choose> 
					   <c:when test="${memberInfo.emailsts_yn=='true' }">
					     <input type="checkbox" name="emailsts_yn"  value="Y" checked /> 본 사이트에서 발송하는 e-mail을 수신합니다.
						</c:when>
						<c:otherwise>
						  <input type="checkbox" name="emailsts_yn"  value="N"  /> 본 사이트에서 발송하는 e-mail을 수신합니다.
						</c:otherwise>
					 </c:choose>
					</td>
					<td>
		...
		<tr>
			<td>
				...
				<input type="button"  value="수정 완료"  src="${contextPath}/main/main.do">
				<input type="hidden" name="command"  value="modify_my_info" /> 
			...
			<td >
				...
				<c:choose>
				  <c:when  test="${member_info.del_yn=='N' }">
				    <input  type="button"  value="회원탈퇴" onClick="fn_delete_member('${mypage_info.member_id }','Y')">
				  </c:when>
