...
function fn_cancel_order(order_id){
	var answer=confirm("예매/예약을 취소하시겠습니까?");
	if(answer==true){
		var formObj=document.createElement("form");
...
<table class="list_view">
		<tbody align=center >
			<tr style="background:#FFA500" >
				<td class="fixed" >예약번호</td>
				<td class="fixed">예약일자</td>
				<td>예약내역</td>
				<td>예약금액</td>
				<td>예약상태</td>
				<td>예약자</td>				
          ...
          <c:when test="${item.order_id != pre_order_id }">   
            <tr>       
				...
				<td>
			     <c:choose>
			   <c:when test="${item.delivery_state=='delivery_prepared'}">
			       <input  type="button" onClick="fn_cancel_order('${item.order_id}')" value="예매/예약취소"  />
			   </c:when>
			   <c:otherwise>
			      <input  type="button" onClick="fn_cancel_order('${item.order_id}')" value="예매/예약취소" disabled />
			   </c:otherwise>
			  </c:choose>
			    </td>
			</tr>
